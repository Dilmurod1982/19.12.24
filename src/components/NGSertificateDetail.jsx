...
export default function NGSertificateDetail() {
  ...
  const [editedDocNumber, setEditedDocNumber] = useState(ngsertificate_number);
  ...
  const handleSave = async () => {
    try {
      ...
      const data = {
        ngsertificate_number: editedDocNumber,
        issue: formatDate(editedIssueDate),
        expiration: formatDate(editedExpirationDate),
      };

      const res = await fetch(`${BASE_URL}/ngsertificates/${id}`, {
        method: "PATCH",
        headers: {
  ...
  return (
    ...
    >
      <div className="flex justify-center">
        <h1 className="font-bold text-3xl">ТАБИИЙ ГАЗ СЕРТИФИКАТИ</h1>
      </div>
      <div className="pl-10 flex flex-col gap-4">
        ...
        <h1>
          <span className="font-bold">Сертификат рақами:</span>{" "}
          {isEditing ? (
            <input
              type="text"
        ...
        <h1>
          <span className="font-bold">Сертификат берилган сана:</span>{" "}
          {isEditing ? (
            <input
              type="date"
        ...
        <h1>
          <span className="font-bold">Сертификат муддати тугаш санаси:</span>{" "}
          {isEditing ? (
            <input
              type="date"
        ...
        <h1>
          <span className="font-bold">Сертификат холати:</span>{" "}
