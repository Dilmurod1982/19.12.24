...
import ProdInsuranceDetail from "./ProdInsuranceDetail";

function ProdInsuranceList({
  id,
  moljal,
  ltd_name,
  station_number,
  ...
  const { text, bgColorClass } = getDocumentStatus(expiration);

  return (
    <tr className={`text-center ${bgColorClass}`}>
      <th>{id}</th>
      <td>{moljal}</td>
...
export default ProdInsuranceList;
