...
export default function AddNewNGSertificate({ setSendingData, sendingData }) {
  ...
  useEffect(() => {
    if (sendingData) {
      setLoading(true);
      registerNGSertificate(user?.access_token, sendingData)
        .then((res) => {
          toast.dismiss();
          ...
          setAddItemModal;
          getNgsertificates(user?.access_token)
            .then(({ data }) => {
              setNgsertificates(data);
            })
            .catch((error) =>
  ...
  }, [sendingData, ngsertificates, user, setAddItemModal, setNgsertificates]);
        ...
        <DialogContent className="h-screen">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">
              Янги табиий газ сертификатини қўшиш:
            </DialogTitle>
            <DialogDescription>
