...
export default function KolonkaDetail() {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/kolonka/${id}`, {
        //zamena
        method: "PATCH",
        headers: {
  ...
  return (
    ...
    >
      <div className="flex justify-center">
        <h1 className="font-bold text-3xl">Колонкалар сертификати</h1>
      </div>
      <div className="pl-10 flex flex-col gap-4">
        ...
        <div>
          <h1>
            <span className="font-bold">Колонкалар тури ва завод рақами:</span>{" "}
          </h1>
          <span className="font-bold">
            Жами колонкалар сони: {kolonki.length}
          </span>{" "}
          {kolonki.map((k) => (
            <p key={k.serialNumber}>
              Маркаси {k.marka}, завод рақами {k.serialNumber}
            </p>
          ))}
        </div>
