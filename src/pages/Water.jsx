...
import AddNewWater from "../components/water/AddNewWater";
...
function Water() {
  ...
  const water = useAppStore((state) => state.water); //zamena
  const setWater = useAppStore((state) => state.setWater); //zamena
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "WATER"); //zamena

    XLSX.writeFile(workbook, "water.xlsx"); //zamena
  };
  return (
    <>
          ...
          <div className="flex justify-between w-full px-4">
            ...
            {user.type === "admin" ? (
              <Button
                ...
                disabled={water ? false : true} //zamena
                className={water ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
      ...
      <AddNewWater sendingData={sendingData} setSendingData={setSendingData} />
