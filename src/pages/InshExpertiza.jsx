...
function InshExpertiza() {
  // замена

  const zusbase = "inshexpertiza";
  const setZusbase = "setInshexpertiza";
  const docName = "Заправка тўлиқ лойиҳаси экспертизаси";
  ...
  useEffect(() => {
    fetchDataWithTokenRefresh(
      () => getDocs(user?.access_token, zusbase), // zamena
      setZusdoc, // zamena
      user,
      setUser
    );
  }, [user, setZusdoc]); // zamena
  ...
  const filteredDocs = () => {
    let filtered = [...zusdoc]; //zamena

    if (!showAllDocs) {
      const latestDocs = filtered.reduce((acc, doc) => {
        const { station_id, issue } = doc;
        const parsedIssue = new Date(parseDate(issue));
        if (
          !acc[station_id] ||
          new Date(parseDate(acc[station_id].issue)) < parsedIssue
        ) {
          acc[station_id] = doc;
        }
        return acc;
    ...
    if (selectedExpirationFilter !== "all") {
      ...
      filtered = filtered.filter((doc) => {
        const issueDate = new Date(parseDate(doc.issue));
        const daysLeft = (issueDate - currentDate) / (1000 * 60 * 60 * 24);
  ...
  const exportToExcel = () => {
    ...
    const data = filteredExcel.map((doc, index) => ({
      ...
      "Хужжат рақами": doc.docNumber, //zamena
      "Берилган сана": doc.issue,
    }));

    const worksheet = XLSX.utils.json_to_sheet(data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, `${zusbase}`); //zamena

    XLSX.writeFile(workbook, `${zusbase}.xlsx`); //zamena
  };
  return (
    <>
          ...
          <div className="flex justify-between w-full px-4">
            ...
            {user.type === "admin" ? (
              <Button
                ...
                disabled={zusdoc ? false : true} //zamena
                className={zusdoc ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
                Янги {docName} қўшиш
              </Button> //zamena
            ) : null}
            ...
            <div className="flex items-end gap-10 justify-between w-full">
              ...
              <div className="flex flex-col justify-center items-center">
                ...
                <div className="flex gap-5">
                  ...
                  <Input
                    type="text"
                    placeholder={`${docName} рақами билан`} //zamena
                    value={searchTerm}
        ...
        <table className="table table-xs">
          <thead>
            <tr>
              ...
              <th className="text-center">{docName} рақами</th>
