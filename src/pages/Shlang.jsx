...
import ShlangList from "../components/shlang/ShlangList";
import AddNewShlang from "../components/shlang/AddNewShlang";
...
function Shlang() {
  ...
  const shlang = useAppStore((state) => state.shlang); //zamena
  const setShlang = useAppStore((state) => state.setShlang); //zamena
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "SHLANG"); //zamena

    XLSX.writeFile(workbook, "shlang.xlsx"); //zamena
  };
  return (
    <>
        ...
        <div className="flex flex-col items-center justify-between gap-1">
          <div className="flex justify-between w-full px-4">
            <h1 className="text-3xl font-bold">
              Газ тўлдириш шлангларини синов далолатномалари
            </h1>
            {user.type === "admin" ? (
              <Button
                onClick={setAddItemModal}
                disabled={shlang ? false : true} //zamena
                className={shlang ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
      ...
      <AddNewShlang sendingData={sendingData} setSendingData={setSendingData} />
