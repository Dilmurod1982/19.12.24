...
import { AddNewNGSertificate, NGSertificatesList } from "../components";
...
function NGSertificates() {
  const [sendingData, setSendingData] = useState(null);
  const [showAllDocs, setShowAllDocs] = useState(true); // Новое состояние для галочки
  ...
  useEffect(() => {
    fetchDataWithTokenRefresh(getStations, setStations, user);
  }, [user, setStations]);

  useEffect(() => {
    fetchDataWithTokenRefresh(getLtd, setLtd, user);
  }, [user, setLtd]);
  ...
  if (!stations || !ngsertificates || !ltd) {
    return (
  ...
  const exportToExcel = () => {
    ...
    const data = filteredDocs.map((doc, index) => ({
      "#": index + 1,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "NGSertificates");

    // Скачивание файла
    XLSX.writeFile(workbook, "ngsertificates.xlsx");
  };
  return (
    <>
          ...
          <div className="flex justify-between w-full px-4">
            ...
            {user.type === "admin" ? (
              <Button
                ...
                disabled={ngsertificates ? false : true}
                className={
                  ngsertificates ? "cursor-pointer" : "cursor-not-allowed"
                }
              >
            ...
            <div className="flex items-end gap-10 justify-between w-full">
              ...
              <div className="flex flex-col justify-center items-center">
                ...
                <div className="flex gap-5">
                  ...
                  <Input
                    type="text"
                    placeholder="Сертификат рақами билан"
                    value={searchTerm}
              ...
              <tr>
                <td colSpan="7" className="text-center">
                  <h1 className="my-5 btn-link text-2xl italic">
                    Табиий газ сертификатлари мавжуд эмас
                  </h1>
                </td>
              </tr>
            )}
