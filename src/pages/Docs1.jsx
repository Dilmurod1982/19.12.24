...
function Docs() {
  ...
  const filterManometr = () => {
    ...
    if (!showAllManometr) {
      //zamena
      const latestManometr = filtered.reduce((acc, manometr) => {
        //zamena zamena
        const { station_id, expiration } = manometr; //zamena
        const parsedExpiration = new Date(parseDate(expiration));
        ...
        if (
          ...
          acc[station_id] = manometr; //zamena
        }
        return acc;
      }, {});
      filtered = Object.values(latestManometr); //zamena
    }
    ...
    if (selectedStation && selectedStation !== "all") {
      filtered = filtered.filter(
        (
          manometr //zamena
        ) => getStationNameByNumber(manometr.station_id) === selectedStation //zamena
      );
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (
          ...
          manometr.docNumber //zamena zamena
            .toLowerCase()
            .includes(searchTerm.toLowerCase())
      );
    }
  ...
  const manometrCounts = filterManometr().reduce(
    //zamena
    (acc, manometr) => {
      //zamena
      const expirationDate = new Date(parseDate(manometr.expiration)); //zamena
      const currentDate = new Date();
  ...
  const filterTermometr = () => {
    ...
    if (!showAllTermometr) {
      //zamena
      const latestTermometr = filtered.reduce((acc, termometr) => {
        //zamena zamena
        const { station_id, expiration } = termometr; //zamena
        const parsedExpiration = new Date(parseDate(expiration));
        ...
        if (
          ...
          acc[station_id] = termometr; //zamena
        }
        return acc;
      }, {});
      filtered = Object.values(latestTermometr); //zamena
    }
    ...
    if (selectedStation && selectedStation !== "all") {
      filtered = filtered.filter(
        (
          termometr //zamena
        ) => getStationNameByNumber(termometr.station_id) === selectedStation //zamena
      );
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (
          ...
          termometr.docNumber //zamena zamena
            .toLowerCase()
            .includes(searchTerm.toLowerCase())
      );
    }
  ...
  const termometrCounts = filterTermometr().reduce(
    //zamena
    (acc, termometr) => {
      //zamena
      const expirationDate = new Date(parseDate(termometr.expiration)); //zamena
      const currentDate = new Date();
  ...
  const filterVoltmetr = () => {
    //zamena
    let filtered = [...voltmetr]; //zamena

    if (!showAllVoltmetr) {
      //zamena
      const latestVoltmetr = filtered.reduce((acc, voltmetr) => {
        //zamena zamena
        const { station_id, expiration } = voltmetr; //zamena
        const parsedExpiration = new Date(parseDate(expiration));
        ...
        if (
          ...
          acc[station_id] = voltmetr; //zamena
        }
        return acc;
      }, {});
      filtered = Object.values(latestVoltmetr); //zamena
    }
    ...
    if (selectedStation && selectedStation !== "all") {
      filtered = filtered.filter(
        (
          voltmetr //zamena
        ) => getStationNameByNumber(voltmetr.station_id) === selectedStation //zamena
      );
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (
          ...
          voltmetr.docNumber //zamena zamena
            .toLowerCase()
            .includes(searchTerm.toLowerCase())
      );
    }
  ...
  const voltmetrCounts = filterVoltmetr().reduce(
    //zamena
    (acc, voltmetr) => {
      //zamena
      const expirationDate = new Date(parseDate(voltmetr.expiration)); //zamena
      const currentDate = new Date();
    ...
    <div className="flex flex-col justify-center items-center gap-5 w-full">
      ...
      <ul className="flex flex-col justify-center items-center gap-3 w-[600px]">
        ...
        <li className="flex justify-center w-full">
          ...
          >
            <Button className="flex justify-between items-center gap-5 w-full text-xl">
              Манометрлар сертификатлари
              <div className="flex gap-8">
                {manometrCounts.active > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-green-500">
                      {manometrCounts.active}
                    </span>
                  </div>
                )}
                {manometrCounts.expiringSoon15 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-yellow-500 ">
                      {manometrCounts.expiringSoon15}
                    </span>
                  </div>
                )}
                {manometrCounts.expiringSoon5 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-orange-500">
                      {manometrCounts.expiringSoon5}
                    </span>
                  </div>
                )}
                {manometrCounts.expired > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-red-700 text-white">
                      {manometrCounts.expired}
                    </span>
                  </div>
                )}
              </div>
        ...
        <li className="flex justify-center w-full">
          ...
          >
            <Button className="flex justify-between items-center gap-5 w-full text-xl">
              Термометрлар сертификатлари
              <div className="flex gap-8">
                {termometrCounts.active > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-green-500">
                      {termometrCounts.active}
                    </span>
                  </div>
                )}
                {termometrCounts.expiringSoon15 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-yellow-500 ">
                      {termometrCounts.expiringSoon15}
                    </span>
                  </div>
                )}
                {termometrCounts.expiringSoon5 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-orange-500">
                      {termometrCounts.expiringSoon5}
                    </span>
                  </div>
                )}
                {termometrCounts.expired > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-red-700 text-white">
                      {termometrCounts.expired}
                    </span>
                  </div>
                )}
              </div>
        ...
        <li className="flex justify-center w-full">
          ...
          >
            <Button className="flex justify-between items-center gap-5 w-full text-xl">
              Амперметр ва вольтметр сертификатлари
              <div className="flex gap-8">
                {voltmetrCounts.active > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-green-500">
                      {voltmetrCounts.active}
                    </span>
                  </div>
                )}
                {voltmetrCounts.expiringSoon15 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-yellow-500 ">
                      {voltmetrCounts.expiringSoon15}
                    </span>
                  </div>
                )}
                {voltmetrCounts.expiringSoon5 > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-orange-500">
                      {voltmetrCounts.expiringSoon5}
                    </span>
                  </div>
                )}
                {voltmetrCounts.expired > 0 && (
                  <div className="indicator ">
                    <span className="badge badge-sm indicator-item bg-red-700 text-white">
                      {voltmetrCounts.expired}
                    </span>
                  </div>
                )}
              </div>
