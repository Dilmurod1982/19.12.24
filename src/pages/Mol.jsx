...
import MolList from "../components/mol/MolList";
import AddNewMol from "../components/mol/AddNewMol";
...
function Mol() {
  ...
  const mol = useAppStore((state) => state.mol); //zamena
  const setMol = useAppStore((state) => state.setMol); //zamena
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "MOL"); //zamena

    XLSX.writeFile(workbook, "mol.xlsx"); //zamena
  };
  return (
    <>
        ...
        <div className="flex flex-col items-center justify-between gap-1">
          <div className="flex justify-between w-full px-4">
            <h1 className="text-3xl font-bold">
              Чақмоқ қайтаргич ва электр кабеллари синов далолатномалари
            </h1>
            {user.type === "admin" ? (
              <Button
                onClick={setAddItemModal}
                disabled={mol ? false : true} //zamena
                className={mol ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
      ...
      <AddNewMol sendingData={sendingData} setSendingData={setSendingData} />
