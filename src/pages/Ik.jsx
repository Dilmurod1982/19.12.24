...
import IkList from "../components/ik/IkList";
import AddNewIk from "../components/ik/AddNewIk";
...
function Ik() {
  ...
  const ik = useAppStore((state) => state.ik); //zamena
  const setIk = useAppStore((state) => state.setIk); //zamena
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "IK"); //zamena

    XLSX.writeFile(workbook, "ik.xlsx"); //zamena
  };
  return (
    <>
        ...
        <div className="flex flex-col items-center justify-between gap-1">
          <div className="flex justify-between w-full px-4">
            <h1 className="text-3xl font-bold">
              Газ ўлов комплекси ИК сертификатлари
            </h1>
            {user.type === "admin" ? (
              <Button
                onClick={setAddItemModal}
                disabled={ik ? false : true} //zamena
                className={ik ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
      ...
      <AddNewIk sendingData={sendingData} setSendingData={setSendingData} />
