...
import PilotList from "../components/pilot/PilotList";
import AddNewPilot from "../components/pilot/AddNewPilot";
...
function Pilot() {
  ...
  const pilot = useAppStore((state) => state.pilot); //zamena
  const setPilot = useAppStore((state) => state.setPilot); //zamena
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "pilot"); //zamena

    XLSX.writeFile(workbook, "pilot.xlsx"); //zamena
  };
  return (
    <>
      <div className="overflow-x-auto">
        <div className="flex flex-col items-center justify-between gap-1">
          <div className="flex justify-between w-full px-4">
            <h1 className="text-3xl font-bold">Автопилот сертификатлари</h1>
            {user.type === "admin" ? (
              <Button
                onClick={setAddItemModal}
                disabled={pilot ? false : true} //zamena
                className={pilot ? "cursor-pointer" : "cursor-not-allowed"} //zamena
              >
      ...
      <AddNewPilot sendingData={sendingData} setSendingData={setSendingData} />
