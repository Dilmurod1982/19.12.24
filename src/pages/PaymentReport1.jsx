...
const PaymentReport = () => {
  ...
  const handleSavePayment = async (values, approvalStatus = "0") => {
    try {
      const token = user?.access_token;
      if (!values.partner_id || !values.station_id) {
        throw new Error("Не указан partner_id или station_id");
      }
      const paymentData = {
      ...
      let reportToUpdate = responseData.data?.[0] || responseData;
      ...
      if (!reportToUpdate) {
        ...
        reportToUpdate = newReportData.data || newReportData; // Исправлено!
        if (!reportToUpdate.id) throw new Error("Сервер не вернул ID отчета");
      }
      ...
      // 2. Обновляем отчет с новым платежом
      const updatedPayment = [
        ...(reportToUpdate.payment || []),
        {
          paymentNumber: values.paymentNumber,
          paymentSum: values.paymentSum,
          approval: approvalStatus,
      ...
      if (!updateResponse.ok) throw new Error("Ошибка обновления отчета");

      // 3. Получаем все последующие отчеты для этого партнера и станции
      const allReportsResponse = await fetch(
        `${BASE_URL}/partnersdailyreports?partner_id=${values.partner_id}&station_id=${values.station_id}&_sort=date&_order=asc`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (!allReportsResponse.ok)
        throw new Error("Ошибка при получении всех отчетов");

      const allReports = await allReportsResponse.json();
      const reportsToUpdate = allReports.data || allReports;

      // Находим индекс отчета с платежом
      const currentReportIndex = reportsToUpdate.findIndex(
        (r) => r.id === reportToUpdate.id
      );

      if (currentReportIndex === -1) {
        throw new Error("Текущий отчет не найден в общем списке");
      }

      // 4. Обновляем все последующие отчеты
      let previousBalance = updatedFinalBalance;
      for (let i = currentReportIndex + 1; i < reportsToUpdate.length; i++) {
        const report = reportsToUpdate[i];

        // Обновляем initial_balance из final_balance предыдущего дня
        const newInitialBalance = previousBalance;
        const totalSum = Number(report.total_sum) || 0;
        const payments = report.payment || [];
        const totalPayment = payments.reduce(
          (sum, p) => sum + Number(p.paymentSum),
          0
        );
        const newFinalBalance = newInitialBalance + totalSum - totalPayment;
        previousBalance = newFinalBalance;

        // Обновляем отчет
        const updateReportResponse = await fetch(
          `${BASE_URL}/partnersdailyreports/${report.id}`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!updateReportResponse.ok) {
          ...
          console.error(
            `Ошибка обновления отчета ${report.id}: ${updateReportResponse.status} - ${errorText}`
          );
          // Продолжаем обновление других отчетов, даже если один не удался
        }
      }

      message.success("Оплата успешно добавлена и выполнены все перерасчеты");
